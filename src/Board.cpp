...
public:
    ...
    std::array<std::array<long, 64>, 12> mPieceSquareTableOpeningMB;
    std::array<std::array<long, 64>, 12> mPieceSquareTableEnding;
    std::array<std::array<long, 64>, 12> mPieceSquareTableEndingMB;
...
//                mPieceSquareTableEnding[p + Color::Black * 6][sq ^ 56] = -(pieceValuesEnding[p]);
    ...
    long getPstSqOpening(int type , int ind){
        if((me == 'b' && (whitePieces & (1ull << ind))) || (me == 'w' && (blackPieces & (1ull << ind)))){
            type = type+6;
        }
        if(me == 'b')ind ^= 56;

        return mPieceSquareTableOpening[type][ind];

    }
    long getPstSqEnding(int type, int ind){
        if((me == 'b' && (whitePieces & (1ull << ind))) || (me == 'w' && (blackPieces & (1ull << ind)))){
            type = type+6;
        }
        if(me == 'b')ind ^= 56;

        return mPieceSquareTableEnding[type][ind];

    }

    void updPst(int type, int from, int to){
        if((me == 'b' && (whitePieces & (1ull << from))) || (me == 'w' && (blackPieces & (1ull << from)))){
            type = type+6;
        }
        if(me == 'b')from ^= 56, to ^=56;


        mPstScoreOp -= mPieceSquareTableOpening[type][from];
        mPstScoreEd -= mPieceSquareTableEnding[type][from];
        mPstScoreOp += mPieceSquareTableOpening[type][to];
        mPstScoreEd += mPieceSquareTableEnding[type][to];


    }
    void updPst(int newType, int from){
        int ot = Piece::Pawn;
        if((me == 'b' && (whitePieces & (1ull << from))) || (me == 'w' && (blackPieces & (1ull << from)))){
            newType = newType+6;
            ot = ot+6;
        ...
        mPstScoreOp -= mPieceSquareTableOpening[ot][from];
        mPstScoreEd -= mPieceSquareTableEnding[ot][from];
        mPstScoreOp += mPieceSquareTableOpening[newType][to];
        mPstScoreEd += mPieceSquareTableEnding[newType][to];


    }
    void capPst(int type, int to){
        if((me == 'b' && (!whiteToMove) || (me == 'w' && (whiteToMove)))){
            type = type+6;
        }
        if(me == 'b')to ^=56;
        mPstScoreOp -= mPieceSquareTableOpening[type][to];
    ...
    void fenInterpreter(string fen = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1", bool start = true) {
        ...
        ull WP = me == 'w' ? whitePawns : blackPawns;
        ull WN = me == 'w' ? whiteKnights : blackKnights;
        ull WB = me == 'w' ? whiteBishops : blackBishops;
        ull WR = me == 'w' ? whiteRooks : blackRooks;
        ull WQ = me == 'w' ? whiteQueens : blackQueens;
        ull WK = me == 'w' ? whiteKing : blackKing;

        ull BP = me == 'b' ? whitePawns : blackPawns;
        ull BN = me == 'b' ? whiteKnights : blackKnights;
        ull BB = me == 'b' ? whiteBishops : blackBishops;
        ull BR = me == 'b' ? whiteRooks : blackRooks;
        ull BQ = me == 'b' ? whiteQueens : blackQueens;
        ull BK = me == 'b' ? whiteKing : blackKing;
...
//            int X = ((inc) ? (0) : (56));
            if ((1ull << ind) & WP) {
                mPstScoreOp += getPstSqOpening(Piece::Pawn, ind);
                mPstScoreEd += getPstSqEnding(Piece::Pawn, ind);

            }
            if ((1ull << ind) & WN) {
                mPstScoreOp += getPstSqOpening(Piece::Knight, ind);
                mPstScoreEd += getPstSqEnding(Piece::Knight, ind);

            }
            if ((1ull << ind) & WR) {
                mPstScoreOp += getPstSqOpening(Piece::Rook, ind);
                mPstScoreEd += getPstSqEnding(Piece::Rook, ind);

            }
            if ((1ull << ind) & WB) {
                mPstScoreOp += getPstSqOpening(Piece::Bishop, ind);
                mPstScoreEd += getPstSqEnding(Piece::Bishop, ind);

            }
            if ((1ull << ind) & WQ) {
                mPstScoreOp += getPstSqOpening(Piece::Queen, ind);
                mPstScoreEd += getPstSqEnding(Piece::Queen, ind);

            }
            if ((1ull << ind) & WK) {
                mPstScoreOp += getPstSqOpening(Piece::King, ind);
                mPstScoreEd += getPstSqEnding(Piece::King, ind);
...
//==================================================Make Move(doo)
    ...
    void doo(int move) {
        ...
        int ex;
        int inc = ((me == 'b' && whiteToMove) || (me == 'w' && !whiteToMove));
        int X = inc ? 56 : 0;
        //int inc;
        switch (type) {
...
//                    mPstScoreEd -= mPieceSquareTableEnding[ex][from];
            ...
            case 1:
                ...
                updPst(Piece::Knight, from, to);
                break;
            case 2:
                refBishops ^= moveXor;
                key ^= ZMove(from, to, mb);

                updPst(Piece::Bishop, from, to);
        ...
        if (whiteToMove) {
            for (int i = 0; i < 8; ++i) {
                if (isToKing(locW, locW + di[i]) && cellInBoard(locW + di[i]) &&
                    locExist(intersection, 1ull << (locW + di[i])))
                    continue;
                if (isToKing(locW, locW + di[i]) && cellInBoard(locW + di[i]))
            ...
            bool check = isCheck();
            if (!check) {
                if (!((allPieces & (1ull << 5)) | (allPieces & (1ull << 6))) && whiteCastleK) {
                ...
                if (!(((allPieces & (1ull << 1)) | (allPieces & (1ull << 2)) | (allPieces & (1ull << 3)))) &&
                    whiteCastleQ) {
            ...
            for (int i = 0; i < 8; ++i) {
                if (isToKing(locB, locB + di[i]) && cellInBoard(locB + di[i]) &&
                    locExist(intersection, 1ull << (locB + di[i])))
                    continue;
                if (isToKing(locB, locB + di[i]) && cellInBoard(locB + di[i]))
            ...
            bool check = isCheck();
            if (!check) {
                if (!((allPieces & (1ull << 61)) | (allPieces & (1ull << 62))) && blackCastleK) {
                ...
                if (!(((allPieces & (1ull << 57)) | (allPieces & (1ull << 58)) | (allPieces & (1ull << 59)))) &&
                    blackCastleQ) {
