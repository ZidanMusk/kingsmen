        ...
        try:
            while 1:
                ...
                if opcode == 2:
                    ...
                    log('[Player' + str(player+1) + '] Recieved move : piece = ' + str(piece) + '  dst = ' + str(dest))
                    ...
                    log('[Player' + str(player+1) + '] Recieved move with promotion : piece = ' + str(piece) + '  dst = ' + str(dest) + ' prom = ' + str(prom))
                ...
                #     try:
                #         Othersock = self.hsockets[(player + 1) % 2]
                #         sock.sendall(pack('is', 3, 'W'))
                #         Othersock.sendall(pack('is', 3, 'L'))
                #     except Exception, e:
                #         # any exception for the other palyer thread should not hang the current thread
                #         print '[-] failed to send win to player %d' % (
                #             (player + 1) % 2 + 1)  # just show the other player num from it's index
                #         print e
                #         continue
                #     raw_input('Done')
...
if __name__ == "__main__":
    ...
    ServerIP = raw_input("Enter server's IP: ")
    print "\n"
    log('Server IP: ' + ServerIP)

    ServerPort = raw_input("Enter server's listening port: ")
    print "\n"
    log('Port: ' + ServerPort)
    ...
    while GameMode not in range(1, 3):
        GameMode = input("Invalid Choice! Your choice: ")
    print "\n"

    print "Choose turns flag:\n\t0- Random player starts\n\t1- Player 1 starts\n\t2- Player2 starts"
    TurnFlag = input("Your choice: ")
    while TurnFlag not in range(0, 3):
        TurnFlag = input("Invalid Choice! Your choice: ")
    ...
    ThreadedServer(str(ServerIP), int(ServerPort), int(GameMode), str(BoardState), 0, 0, int(TurnFlag)).listen()
