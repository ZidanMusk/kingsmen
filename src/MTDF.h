...
private:
    ...
    unordered_map<int,vector<int>>_DeepeningTableLastIter;
    unordered_map<int,vector<int>>_DeepeningTableCurIter;
    pair<int,int> _IterativeDeepening(int root_id,int MaxDepth);
    int _MTDF(int root_id, int f, int d);
    int _AlphaBetaWithMemory(int state_id, int alpha, int beta,int d,bool IsMax,int MaxDepth);
    void _UpdateNode(int state_id,int g ,int alpha, int beta);
    vector<int> _GetSortedPossibleMoves(int state_id);
